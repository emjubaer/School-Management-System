    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-6">
        ...
        <div class="md:col-span-7 bg-white dark:bg-gray-800 rounded-lg shadow">
            ...
            <div class="p-5 space-y-6">
                @foreach($classRooms as $classRoom)
                    <div>
                        <h3 class="text-indigo-600 font-semibold mb-2">
                            {{ $classRoom->name }}
                        </h3>
                                ...
                                <table class="w-full text-sm border">
                                    <thead class="bg-gray-100 dark:bg-gray-700">
                                        <tr>
                                            ...
                                            <th class="border px-3 py-2">Full</th>
                                            <th class="border px-3 py-2">Pass</th>
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach($classRoom->subjects as $subject)
                                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                                <td class="border px-3 py-2">{{ $subject->sub_code }}</td>
                                                <td class="border px-3 py-2">{{ $subject->name }}</td>
                                                <td class="border px-3 py-2">{{ $subject->fullmark }}</td>
                                                <td class="border px-3 py-2">{{ $subject->pass_mark }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
